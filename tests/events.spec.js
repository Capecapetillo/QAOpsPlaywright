...
/**
 * Logs in via the UI and then navigates to the /events page.
...
async function loginAndGoToEvents(page) {
  ...
  // Wait until the app has settled after login before going to /events
...
test.describe('EventHub — banner visibility based on event count', () => {
 
 // ── Test 1 ──────────────────────────────────────────────────────────────
test('Banner IS visible when 6 events are returned', async ({ page }) => {
 
  await page.route(
    (url) => url.href.includes('/api/events'),
    (route) => {
      route.fulfill({
        status: 200,
